...
-- =============================================
-- Tabela POZIVI - Struktura i RLS politike
-- =============================================
-- NAPOMENA: Tabela 'pozivi' treba biti kreirana u bazi
-- Pokrenite ove komande u Supabase SQL Editor-u

-- Kreiranje tabele pozivi (ako ne postoji)
/*
CREATE TABLE public.pozivi (
  id bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  ...
  email text NULL,
  imekupca text NULL,
  drzava character varying NULL,
  regija text NULL,
  validacija_ag text NULL,
  kodkampanje text NULL,
  ponudaid bigint NULL,
  CONSTRAINT pozivi1_pkey PRIMARY KEY (id),
  CONSTRAINT pozivi1_ponudaid_fkey FOREIGN KEY (ponudaid) REFERENCES ponuda(id) ON DELETE RESTRICT
) TABLESPACE pg_default;
*/

-- Omogući RLS za pozivi tabelu
ALTER TABLE pozivi ENABLE ROW LEVEL SECURITY;

-- Politika za čitanje poziva (svi mogu da čitaju)
DROP POLICY IF EXISTS "Allow public read pozivi" ON pozivi;
CREATE POLICY "Allow public read pozivi" ON pozivi
  FOR SELECT USING (true);

-- Politika za insert poziva (svi mogu da dodaju - provera uloge se radi u aplikaciji)
DROP POLICY IF EXISTS "Allow public insert pozivi" ON pozivi;
CREATE POLICY "Allow public insert pozivi" ON pozivi
  FOR INSERT WITH CHECK (true);

-- Politika za update poziva (svi mogu da ažuriraju - provera uloge se radi u aplikaciji)
DROP POLICY IF EXISTS "Allow public update pozivi" ON pozivi;
CREATE POLICY "Allow public update pozivi" ON pozivi
  FOR UPDATE USING (true) WITH CHECK (true);

-- Politika za delete poziva (svi mogu da brišu - provera uloge se radi u aplikaciji)
DROP POLICY IF EXISTS "Allow public delete pozivi" ON pozivi;
CREATE POLICY "Allow public delete pozivi" ON pozivi
  FOR DELETE USING (true);
